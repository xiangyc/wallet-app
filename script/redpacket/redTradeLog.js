var tradeType = '';
var period = 4;
var url = global.getRequestUri() + '/envelope/tradeLog';
...
function queryTradeLogType() {
	api.ajax({
            url: global.getRequestUri() + '/options?name=GoldEnvelopeTradeType',
            method: 'get',
            timeout: 30,
            dataType: 'json',
            returnAll: false,
...
function queryTradeLog() {
	var params = '?tradeType=' + tradeType + '&period=' + period + '&start=0&maxResults=15';
	page.init(15, 'tradeLog-content', 'tradeLog-template', url, params, true, 'no-records');
}