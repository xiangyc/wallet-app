...
var envelopeAmount,amount,quantity,remark,url,lastPrice,payPwd;
var redpacketType = 2;
var themeCode = '';
...
apiready = function() {
	...
	if(api.pageParam.defaultData){
		var defaultData = api.pageParam.defaultData;
		$api.val($api.byId('remark'), defaultData[0].greetings);
		if(defaultData[0].amount){
			$api.val($api.byId('amount'), defaultData[0].amount);
		}
	}
...
function changeType(type){
	...
	$api.val($api.byId('remark'),remark);
...
function next(){
	if(verifyRedpacketRule()){
        ...
        // 发放总克重
        var payMoney = global.formatNumber(amount,3);
        //手气红包
        if(redpacketType === 2){
            perAmount = calculate.div(amount,quantity);
        } else {
        	payMoney = global.formatNumber(calculate.mul(amount,quantity),3);
        }
	...
	api.ajax({
        ...
        if(ret){
            if(ret.success){
				...
				var params = { "page" : page, "title" : "黄金红包" ,"amount":amount,"quantity":quantity,"remark":ret.data.remark,"securityCode":ret.data.securityCode,"lastPrice":lastPrice};
